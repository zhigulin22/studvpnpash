...
def get_devices(telegram_id):
    conn = sqlite3.connect(file_name)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM devices WHERE telegram_id = ?", (telegram_id,))
    devices = cursor.fetchall()
    conn.close()
    return devices

def add_device(telegram_id, device, subscription_end_time, vless_link):
    conn = sqlite3.connect(file_name)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO devices (telegram_id, device, subscription_end_time, vless_link) VALUES (?, ?, ?, ?)",
                   (telegram_id, device, subscription_end_time.isoformat(), vless_link))
    conn.commit()
    conn.close()

def update_device_subscription(telegram_id, device, subscription_end_time):
    conn = sqlite3.connect(file_name)
    cursor = conn.cursor()
    cursor.execute("UPDATE devices SET subscription_end_time = ? WHERE telegram_id = ? AND device = ?",
                    (subscription_end_time.isoformat(), telegram_id, device))
    conn.commit()
    conn.close()
