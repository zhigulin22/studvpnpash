...
async def create_database():
    ...
    try:
        ...
        # Создаем таблицу user_referrals с новыми колонками
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS user_referrals (
            telegram_id INTEGER PRIMARY KEY,
            user_name TEXT,  -- Заменяем message_id на user_name
            referral_count INTEGER DEFAULT 0,
            referal_in INTEGER DEFAULT 0,  -- Добавляем новую колонку
        ...
        # Остальной код создания таблиц остается без изменений
        cursor.execute("""
...
async def add_user(telegram_id, user_name, referral_count, is_agree, referrer_id, referal_in=0):
    ...
    try:
        ...
        cursor.execute("""
        INSERT INTO user_referrals (telegram_id, user_name, referral_count, referal_in, is_agree, referrer_id)
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(telegram_id) DO UPDATE SET
            user_name=excluded.user_name,
            referral_count=excluded.referral_count,
            referal_in=excluded.referal_in,
            is_agree=excluded.is_agree,
            referrer_id=excluded.referrer_id
        """, (telegram_id, user_name, referral_count, referal_in, is_agree, referrer_id))
...
async def get_user_data(telegram_id):
    ...
    try:
        ...
        cursor.execute("""
        SELECT user_name, referral_count, referal_in, is_agree, referrer_id 
        FROM user_referrals 
        WHERE telegram_id = ?
        """, (telegram_id,))

        referral_result = cursor.fetchone()
        user_name = referral_result[0] if referral_result else None
        referral_count = referral_result[1] if referral_result else 0
        referal_in = referral_result[2] if referral_result else 0
        ...
        for i in range(1, DEVICE_LIMIT + 1):
            device_data[f"device{i}"] = None

        for device in devices:
            device_index = device[1]
            device_data[f"device{device_index}"] = {
...
# Обновить кол-во человек зашедших по ссылке
async def update_referal_in(telegram_id, new_count):
    """Updates a user's referal_in count."""
    conn = None
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()
...
# Добавляем функцию для получения username
async def get_username(telegram_id):
    """Retrieves the username for a user based on their telegram_id."""
    conn = None
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()
...
#Получить кол-во зарегистрированых
async def get_referal_in_count(telegram_id):
    """Retrieves the number of incoming referrals for a user."""
    conn = None
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()
        ...
        if result is not None:
            return result[0]  # Возвращаем количество входящих рефералов
        else:
...
#Получить по юзернайм тг айди
async def get_telegram_id_by_username(username):
    """Retrieves telegram_id by username."""
    conn = None
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()

        # Удаляем символ @ если он есть в начале username
        username = username.replace("@", "")

        # Ищем пользователя по username (нечувствительно к регистру)
        cursor.execute("""
        SELECT telegram_id 
        FROM user_referrals 
        ...
        if result:
            return result[0]  # Возвращаем telegram_id
        else:
            return None  # Пользователь не найден
